...
void editorDrawStatusBar(struct abuf *ab) {
  ...
  int len = snprintf(status, sizeof(status), "%.20s - %d lines %s",
    ...
    if(E.cy+1<=5){
     ...
     //int rlen = snprintf(rstatus, sizeof(rstatus), "s->%s sum->%d", temp, sum);
...
*/
    ...
    case ENTER:
      if(isdigit(temp[E.cx]) && (E.cy==1) && (E.cx!=0)){
        E.change_count++;
      	editorDelChar();
      	editorInsertChar('N');
...
int main(int argc, char *argv[]) {
  ...
  editorSetStatusMessage("HELP: Esc = quit ENTER = edit NUM = digit");
